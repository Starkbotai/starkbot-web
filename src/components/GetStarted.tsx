...
export function GetStarted() {
  ...
  return (
    <section className="py-28 px-6 bg-white/[0.01]">
      <div ref={sectionRef} className="scroll-reveal max-w-5xl mx-auto text-center">
        <div
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              transform: showCloud ? 'scaleX(1)' : 'scaleX(0)',
          ...
          >
            <div className="flex items-center justify-center gap-3 mb-4">
              <h3 className="text-2xl sm:text-3xl font-display font-bold text-[#FBFBFB]">
                Deploy to the Cloud
              </h3>
              <button
                onClick={handleFlip}
                className="p-2 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] hover:border-white/[0.12] transition-all duration-300 group"
                title="Show bare metal deploy"
              >
                <RefreshCw className="w-5 h-5 text-[#6a6a6b] group-hover:text-[#FBFBFB] transition-colors" />
              </button>
            </div>
            <p className="text-[#6a6a6b] mb-8 max-w-2xl mx-auto">
              Deploy StarkBot to your favorite cloud platform for a fully managed experience
            </p>

            <div className="bg-white/[0.03] backdrop-blur-sm rounded-2xl border border-white/[0.08] p-8 max-w-2xl mx-auto">
              <div className="flex items-center justify-center gap-3 mb-6">
                <Cloud className="w-8 h-8 text-[#6a6a6b]" />
                <span className="text-xl font-bold text-[#FBFBFB]">One-Click Deploy</span>
              </div>

              <ol className="text-left text-[#6a6a6b] space-y-4 mb-8">
                <li className="flex gap-3">
                  <span className="text-[#3a3a3b] font-bold">1.</span>
                  <span>Click a deploy button below to start</span>
                </li>
                <li className="flex gap-3">
                  <span className="text-[#3a3a3b] font-bold">2.</span>
                  <span>Connect your GitHub account when prompted</span>
                </li>
                <li className="flex gap-3">
                  <span className="text-[#3a3a3b] font-bold">3.</span>
                  <span>Configure environment variables (API keys, DeFi Relay x402 facilitator)</span>
                </li>
                <li className="flex gap-3">
                  <span className="text-[#3a3a3b] font-bold">4.</span>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <a
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-violet-500 text-white font-semibold rounded-lg transition-all duration-300 hover:opacity-90"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] text-[#FBFBFB] font-semibold rounded-lg transition-all duration-300"
                >
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
              {bareMetalSteps.map((step) => (
                <div key={step.number}>
                  <div className="w-12 h-12 border-2 border-white/[0.12] rounded-full flex items-center justify-center text-xl font-bold mx-auto mb-4 text-[#FBFBFB]">
                    {step.number}
                  </div>
                  <h3 className="text-xl font-bold mb-2 text-[#FBFBFB]">{step.title}</h3>
                  <p className="text-[#6a6a6b]">{step.description}</p>
                </div>
              ))}
            </div>

            <div className="bg-black/50 rounded-xl border border-white/[0.08] p-6 text-left max-w-2xl mx-auto">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-3 h-3 bg-white/[0.12] rounded-full" />
                <div className="w-3 h-3 bg-white/[0.12] rounded-full" />
                <div className="w-3 h-3 bg-white/[0.12] rounded-full" />
                <span className="ml-2 text-[#3a3a3b] text-sm font-mono">terminal</span>
              </div>
              <code className="text-sm text-[#FBFBFB] font-mono block space-y-1">
                <div><span className="text-[#3a3a3b]">$</span> git clone https://github.com/ethereumdegen/stark-bot</div>
                <div><span className="text-[#3a3a3b]">$</span> cd stark-bot</div>
                <div><span className="text-[#3a3a3b]">$</span> cp .env.example .env</div>
                <div><span className="text-[#3a3a3b]">$</span> docker compose up -d</div>
              </code>
            </div>
          </div>
        </div>
